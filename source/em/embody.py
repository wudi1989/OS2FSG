...
def _em_step_body_row(Z_row, r_lower_row, r_upper_row, sigma, num_ord_updates=1):
    ...
    C = np.zeros((p,p))
    obs_indices = np.where(~np.isnan(Z_row))[0] #按行输出满足条件的元素的坐标

    missing_indices = np.setdiff1d(np.arange(p), obs_indices)   #找到两个数组中集合元素的差异。返回在ar1中但不在ar2中已排序的唯一值
    ...
    sigma_obs_obs = sigma[np.ix_(obs_indices,obs_indices)]   #输入两个数组，产生笛卡尔积的关系
    ...
    if len(missing_indices) > 0:
        tot_matrix = np.concatenate((np.identity(len(sigma_obs_obs)), sigma_obs_missing), axis=1) #np.identity 创建对角线为1的矩阵
        intermed_matrix = np.linalg.solve(sigma_obs_obs, tot_matrix)        #以矩阵形式解线性方程，AX=b，返回X
    ...
    if len(missing_indices) > 0:
        Z_imp_row[missing_indices] = np.matmul(J_obs_missing.T,Z_obs)   #两矩阵相乘
        ...
        if len(ord_obs_indices) >= 1 and len(obs_indices) >= 2 and np.sum(var_ordinal) > 0: 
            ...
            C[np.ix_(missing_indices, ord_obs_indices)] += cov_missing_obs_ord  #输入两个数组，产生笛卡尔积的映射关系，进行索引
