...
def get_tra_zero_datastream(dataset):
    ...
    feat = X.shape[1]
    perm = np.arange(n)
    np.random.seed(1)       #生成同样随机数
    np.random.shuffle(perm)     #改变列表perm顺序
    perm = np.array(perm)
    X = X[perm]     
    X = X.tolist()      #数组或矩阵转为列表
    star_row = 0
    end_column = 0
    X_trapezoid = []
    X_masked = []
    ...
    if not os.path.exists(path):    #查看括号内的文件是否存在
...
def chack_Nan(X_masked,n):
    for i in range(n):
        X_masked[i] = X_masked[i].strip()       #默认去掉左右两边的空格
        X_masked[i] = X_masked[i].strip("[]")
        X_masked[i] = X_masked[i].split(",")      #以，分开
        X_masked[i] = list(map(float, X_masked[i]))
        narry = np.array(X_masked[i])
        where_are_nan = np.isnan(narry)     #判断是否是空值
...
def get_cont_indices(X):
    max_ord=14
    indices = np.zeros(X.shape[1]).astype(bool) #转换为Bool类型值
    for i, col in enumerate(X.T):
        col_nonan = col[~np.isnan(col)]
        col_unique = np.unique(col_nonan)   #去除数组中重复数字，排序并输出
...
def mask_types(X, mask_num, seed):    #随机缺失
    X_masked = np.copy(X)
    mask_indices = []
    ...
    for i in range(num_rows):
        ...
        for j in range(num_cols//2):
            rand_idx=np.random.choice(2,mask_num,False) #从随机生成长度为mask_num的0，1，false不能取相同数字
            for idx in rand_idx:
                X_masked[i,idx+2*j]=np.nan
                mask_indices.append((i, idx+2*j))
    return X_masked

def mask_types_p(X, p, seed):    #随机缺失
    ...
    for j in range(num_cols):
        t = np.sum(np.isnan(X_masked[:,j]))
        if t == num_rows:
            i_fill1 = np.random.randint(0,num_rows)
            i_fill2 = np.random.randint(0, num_rows)
            X_masked[i_fill1,j] = X[i_fill1,j]
            X_masked[i_fill2,j] = X[i_fill2,j]
    return X_masked
