...
def draw_cap_error_picture(Z_imp_CER_ran,Z_imp_CER_semi,Z_imp_CER,svm_error,dataset):
    n=len(Z_imp_CER)
    ...
    plt.plot(x, Z_imp_CER_ran  , color='green',label = "Z_imp_CER_adp")    # the error of z_imp
    plt.plot(x, Z_imp_CER_semi, color='blue',label = "Z_imp_CER_semi")     # the error of x_zero
    plt.plot(x,Z_imp_CER, color='m', label="Z_imp_CER")
    plt.plot(x, [svm_error] * n ,color='red') # the error of svm
    plt.legend(loc = "best")

    plt.title("The error of label_imp_CER,label_ran_CER,SVM_CER in "+dataset)
    plt.show()
    plt.clf()

def draw_tra_error_picture(error_arr_Z,error_arr_X,dataset):
    n=len(error_arr_Z)
    ...
    x = range(n)
    plt.plot(x, error_arr_Z, color='green',label = "error_arr_Z")   # z_imp error picture
    plt.plot(x, error_arr_X, color='blue',label = "error_arr_X")    # x_imp error picture
    plt.legend(loc = "best")

    plt.title("The CER of trapezoid data stream in"+dataset)
    plt.show()
    plt.clf()