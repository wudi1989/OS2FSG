...
class TransformFunction():
    ...
    def get_ord_latent(self):
        ...
        for i, x_col in enumerate(X_ord.T):
            ...
            if unique.shape[0] > 1:
                ...
                Z_ord_lower[~missing,i] = norm.ppf(ecdf(x_col[~missing] - threshold))
                Z_ord_upper[~missing,i] = norm.ppf(ecdf(x_col[~missing] + threshold))
                # re-add the nan values
            else:
                Z_ord_lower[~missing, i] = -np.inf
                Z_ord_upper[~missing, i] = np.inf
    ...
    def impute_cont_observed(self, Z):
        ...
        for i, x_col in enumerate(X_cont.T):
            ...
            X_imp[missing,i] = np.quantile(x_col[~missing], norm.cdf(Z_cont[missing,i]))  #np.quantile q分位间距
        return X_imp
